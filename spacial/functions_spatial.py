...
def to_seqgeq_2(adata, header_sg="/home/marine-louarn/Documents/Header_SeqGeq.txt",file=str,genelist=list):
    with open(file, "w") as f:
        ...
        leiden_myelo = adata.obs['leiden_IC2_PC10']#_myelo']
        ...
        f.write("leiden_IC2PC10" + "\t" + "\t".join(leiden_myelo_str) + "\n")
...
def to_seqgeq_myelo(adata, header_sg="/home/marine-louarn/Documents/Header_SeqGeq.txt",file=str,genelist=list):
    with open(file, "w") as f:
        # write header
        with open(header_sg) as header:
            f.write(header.read())
        ...
        umap_y = adata.obsm['X_umap'][:,1]
        umap_y_str = [str(y) for y in umap_y]
        umap_icpc_x = adata.obsm['umapICPC_full'][:,0]
        umap_icpc_x_str = [str(x) for x in umap_icpc_x]
        umap_icpc_y = adata.obsm['umapICPC_full'][:,1]
        umap_icpc_y_str = [str(y) for y in umap_icpc_y]
        umap_icpc2_10_x = adata.obsm['UMAP_IC2_PC10'][:,0]
        umap_icpc2_10_x_str = [str(x) for x in umap_icpc2_10_x]
        umap_icpc2_10_y = adata.obsm['UMAP_IC2_PC10'][:,1]
        umap_icpc2_10_y_str = [str(y) for y in umap_icpc2_10_y]
        #umap_deg_x = adata.obsm['UMAP_wo_DEG'][:,0]
        #umap_deg_x_str = [str(x) for x in umap_deg_x]
        #umap_deg_y = adata.obsm['UMAP_wo_DEG'][:,1]
        #umap_deg_y_str = [str(y) for y in umap_deg_y]
        spatial_x = adata.obsm['spatial'][:,0]
        spatial_x_str = [str(x) for x in spatial_x]
        spatial_y = adata.obsm['spatial'][:,1]
        spatial_y_str = [str(y) for y in spatial_y]
        ...
        f.write("umap_PC1" + "\t" + "\t".join(umap_x_str)+"\n")
        f.write("umap_PC2" + "\t" + "\t".join(umap_y_str)+"\n")
        f.write("umapICPC_full" + "\t" + "\t".join(umap_icpc_x_str)+"\n")
        f.write("umapICPC_full" + "\t" + "\t".join(umap_icpc_y_str)+"\n")
        f.write("umap_IC2PC10_1" + "\t" + "\t".join(umap_icpc2_10_x_str)+"\n")
        f.write("umap_IC2PC10_2" + "\t" + "\t".join(umap_icpc2_10_y_str)+"\n")
        #f.write("umap_wo_DEG" + "\t" + "\t".join(umap_deg_x_str)+"\n")
        #f.write("umap_wo_DEG" + "\t" + "\t".join(umap_deg_y_str)+"\n")
        f.write("Spatial_X" + "\t" + "\t".join(spatial_x_str)+"\n")
        f.write("Spatial_Y" + "\t" + "\t".join(spatial_y_str)+"\n")
        f.write("leiden_clusters_PC" + "\t" + "\t".join(adata.obs['leiden']) + "\n")
        f.write("leiden_ICPC_full" + "\t" + "\t".join(adata.obs['leiden_ICPC_full']) + "\n")
        f.write("leiden_IC2PC10" + "\t" + "\t".join(leiden_myelo_str) + "\n")
        #f.write("leiden_wo_DEG" + "\t" + "\t".join(adata.obs['leiden_wo_DEG']) + "\n")
        f.write("nCount" + "\t" + "\t".join(n_counts_str) + "\n")



def to_seqgeq(adata, header_sg="/home/marine-louarn/Documents/Header_SeqGeq.txt",file=str,genelist=list):
    with open(file, "w") as f:
        # write header
        with open(header_sg) as header:
            f.write(header.read())
...
def to_seqgeq_ingest(adata, header_sg="/home/marine-louarn/Documents/Header_SeqGeq.txt",file=str,genelist=list,myelo=bool):
    with open(file, "w") as f:
        # write header
        with open(header_sg) as header:
            f.write(header.read())
        ...
        umap_y_str = [str(y) for y in umap_y]
        if myelo:
            umap_icpc_x = adata.obsm['UMAP_IC2_PC10'][:,0]
            umap_icpc_x_str = [str(x) for x in umap_icpc_x]
            umap_icpc_y = adata.obsm['UMAP_IC2_PC10'][:,1]
            umap_icpc_y_str = [str(y) for y in umap_icpc_y]
        else:
            umap_icpc_x = adata.obsm['umapICPC'][:,0]
            umap_icpc_x_str = [str(x) for x in umap_icpc_x]
        ...
        n_counts = adata.obs['n_counts']
        n_counts_str = [str(y) for y in n_counts]
        f.write("umap_sample2_1" + "\t" + "\t".join(umap_x_str)+"\n")
        f.write("umap_sample2_2" + "\t" + "\t".join(umap_y_str)+"\n")
        f.write("umap_ICPC1" + "\t" + "\t".join(umap_icpc_x_str)+"\n")
        f.write("umap_ICPC2" + "\t" + "\t".join(umap_icpc_y_str)+"\n")
        f.write("Spatial_X" + "\t" + "\t".join(spatial_x_str)+"\n")
        f.write("Spatial_Y" + "\t" + "\t".join(spatial_y_str)+"\n")
        ...
        f.write("Sample_nb" + "\t" + "\t".join(adata.obs['Sample_nb']) + "\n")


def h5ad_to_tsv(adata,output_basename=str,umap_name=str):
    import pandas as pd
    ...
    df_umap = pd.DataFrame(adata.obsm[umap_name], columns = [f"{umap_name}_1", f"{umap_name}_2"])
