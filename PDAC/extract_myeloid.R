...
for (folder_GSE in folders) {
  ...
  nb_cell = length(colnames(int.integrated))
  print(paste0('nb cells total: ',nb_cell))
  
  cell.myelo <- subset(int.integrated, subset = CellName %in% cell_tokeep)
  nb_cell = length(colnames(cell.myelo))
  print(paste0('nb cells myeloid: ',nb_cell))
  saveRDS(cell.myelo, paste0("Datasets/",folder_GSE,"_myeloid.rds"))
}

saveRDS(obj_seurat, paste0("Datasets/",folder_GSE,"_myeloid_analysis.rds"))
seqgeq_file(obj_seurat,signature_human,paste0("Datasets/",folder_GSE,"_myeloid_analysis.txt"))
